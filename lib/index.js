...
function UPS(args){
    ...
    function buildShipRequest(data, options, resource, callback) {
        soap.createClient(path.join(__dirname,  'wsdl', resource.wsdl), {endpoint: $scope.hosts[$scope.options.environment] + resource.path}, function(err, client) {
          ...
          client.ProcessShipConfirm(params, function(err, result, originalReply) {        
            if($scope.options.debug) {          
              switch( $scope.options.debugOutput ){
                case "json":
                  parser.parseString(client.lastRequest, {explicitArray: false}, function(err, debug) {
    ...
    function buildShipmentAcceptRequest(data, options, resource, callback) {
      if(!options) {
        options = {};
      }
      soap.createClient(path.join(__dirname,  'wsdl', resource.wsdl), {endpoint: $scope.hosts[$scope.options.environment] + resource.path}, function(err, client) {
        ...
        client.ProcessShipAccept(params, function(err, result, originalReply) {        
          if($scope.options.debug) {          
            switch( $scope.options.debugOutput ){
              case "json":
                parser.parseString(client.lastRequest, {explicitArray: false}, function(err, debug) {
    ...
    function handleShipmentAcceptResponse(res, callback) {
      return callback(null, res);
    }
    ...
    var resources = {
        ...
        ship: {f: buildShipRequest, r: handleShipResponse, wsdl: 'ship/Ship.wsdl', path: '/Ship', version: {ServiceId: 'crs', Major: "v1.1", Intermediate: 0, Minor: 0}},
        accept: {f: buildShipmentAcceptRequest, r: handleShipmentAcceptResponse, wsdl: 'ship/Ship.wsdl', path: '/Ship', version: {ServiceId: 'crs', Major: "v1.1", Intermediate: 0, Minor: 0}},
    };
